...
#model1 = lm(X2D_VARV ~ poly(X2D_LARV, 3), data = data3)
model <- lm(hh ~ gg)
...
Scatter = ggplot(data3, aes(x = gg, y = hh))+
  #expand_limits(x=c(-13.5,-0.5),y=c(-15,240))+
  geom_point(shape = 21, size = 3.8, fill = "#316165", position="jitter",alpha=0.8)+
  stat_smooth(method = "lm",formula = y ~ x, 
              size = 1.2, linetype = 1, alpha = 0.7, color = "#3E608D", fill = "#51A3A9")+
  ...
  annotate("text", x = min(gg) + 2, y = max(hh), 
...
p = ggMarginal(Scatter,type="density",xparams = list(fill = scales::alpha("#316165", alpha = 0.8)),size = 12,
               yparams = list(fill = scales::alpha("#316165", alpha = 0.8)))
p
ggsave("LARV_VARV.pdf", p, width = 6, height = 6, unit = "in", dpi = 900)
