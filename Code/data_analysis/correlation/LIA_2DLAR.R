...
p1 = ggplot(dataJC, aes(x = aa, y = bb))+
  ...
  geom_point(shape = 21, size = 3.8, fill = "#732F51", position="jitter",alpha=0.8)+
  stat_smooth(method = "lm",formula = y ~ x, 
              size = 1.2, linetype = 1, alpha = 0.7, color = "#3E608D", fill = "#EDBDC5")+
...
p1 = ggMarginal(p1,type="density",xparams = list(fill = scales::alpha("#732F51", alpha = 0.5)),size = 12,
                yparams = list(fill = scales::alpha("#732F51", alpha = 0.5)))
p1
ggsave("LIA_2DLAR.pdf", p1, width = 6, height = 6, unit = "in", dpi = 900)
