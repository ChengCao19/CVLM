# ============================================================================
# Training loss curves for Deeplabv3+, U-Net, PSPNet, HRNet on TVDs
# Output: PDF (cairo)
# ============================================================================
...
# --- Data preparation ---
loss_df <- T_tra[, c("Step", "Deeplabvplus", "Unet", "PSPNet", "HRNet")]
...
p <- ggplot(loss_long, aes(x = Epochs, y = Loss, color = Model)) +
  ...
  scale_x_continuous(
    breaks = seq(0, max(T_tra$Step), by = 25),
    limits = c(0, max(T_tra$Step)),
    expand = c(0.02, 0),
