# ============================================================================
# Validation loss curves for Deeplabv3+, U-Net, PSPNet, HRNet on SVDs
# Output: PDF (cairo)
# ============================================================================
...
# --- Data preparation ---
loss_df <- S_val[, c("Step", "Deeplabvplus", "Unet", "PSPNet", "HRNet")]
...
p <- ggplot(loss_long, aes(x = Epochs, y = Loss, color = Model)) +
  ...
  scale_x_continuous(
    breaks = seq(0, max(S_val$Step), by = 25),
    limits = c(0, max(S_val$Step)),
    expand = c(0.02, 0),
