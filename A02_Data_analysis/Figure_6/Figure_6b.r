# ============================================================================
# Validation loss curves for Box Loss, Class Loss, Distribution Focal Loss on TVDs
# Output: PDF (cairo)
# ============================================================================
...
# --- Data preparation for loss curves ---
loss_df <- data3[, c("Epochs", "Box.Loss", "Class.Loss", "Distribution.Focal.Loss")]
...
p <- ggplot(loss_long, aes(x = Epochs, y = value, color = Metric)) +
  ...
  scale_x_continuous(
    breaks = seq(0, max(data3$Epochs), by = 25),
    limits = c(0, max(data3$Epochs)),
    expand = c(0.02, 0),
  ...
  labs(x = "Epochs", y = "Validation loss in TVDs") +
