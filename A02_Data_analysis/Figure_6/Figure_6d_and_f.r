# ============================================================================
# Training loss and metric curves for SVDs (Box Loss, Class Loss, Dist. Focal Loss,
...
# ---------- Loss plot ----------
loss_df <- data2[, c("Epochs", "Box.Loss", "Class.Loss", "Distribution.Focal.Loss")]
...
p_loss <- ggplot(loss_long, aes(x = Epochs, y = value, color = Metric)) +
  ...
  scale_x_continuous(
    breaks = seq(0, max(data2$Epochs), by = 25),
    limits = c(0, max(data2$Epochs)),
    expand = c(0.02, 0),
...
# ---------- Metric plot ----------
metric_df <- data2[, c("Epochs", "mAP.50", "mAP.50.95", "Precision", "Recall")]
...
p_metric <- ggplot(metric_long, aes(x = Epochs, y = value, color = Metric)) +
  ...
  labs(x = "Epochs", y = "Training metrics in SVDs") +
  theme_minimal(base_size = 10) +
  theme(
    panel.grid        = element_blank(),
    panel.border      = element_rect(color = "#333333", fill = NA, linewidth = 0.4),
